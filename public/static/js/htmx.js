...
(function (root, factory) {
    ...
    return (function () {
        ...
        function getAttributeValueWithDisinheritance(initialElement, ancestor, attributeName) {
            ...
            if (initialElement !== ancestor && disinherit && (disinherit === "*" || disinherit.split(
        ...
        function findAttributeTargets(elt, attrName) {
            ...
            if (attrTarget) {
                if (attrTarget === "this") {
                    ...
                    if (result.length === 0) {
                        logError('The selector "' + attrTarget + '" on ' + attrName +
                            " returned no matches!");
                        return [DUMMY_ELT]
                    } else {
                        return result;
            ...
            if (tokens[0] === '[') {
                ...
                while (tokens.length > 0) {
                    ...
                    if (isPossibleRelativeReference(token, last, paramName)) {
                        conditionalSource += "((" + paramName + "." + token + ") ? (" + paramName + "." +
                            token + ") : (window." + token + "))";
                    } else {
                        ...
                        if (trigger === "every") {
                            ...
                            while (tokens.length > 0 && tokens[0] !== ",") {
                                ...
                                if (token === "changed") {
                                    ...
                                    tokens.shift();
                                    triggerSpec.delay = parseInterval(consumeUntil(tokens,
                                        WHITESPACE_OR_COMMA));
                                } else if (token === "from" && tokens[0] === ":") {
                                    tokens.shift();
                                    var from_arg = consumeUntil(tokens, WHITESPACE_OR_COMMA);
                                    if (from_arg === "closest" || from_arg === "find" || from_arg ===
                                        "next" || from_arg === "previous") {
                                        tokens.shift();
                                        from_arg +=
                                            " " +
                                            consumeUntil(
                                    ...
                                    tokens.shift();
                                    triggerSpec.throttle = parseInterval(consumeUntil(tokens,
                                        WHITESPACE_OR_COMMA));
        ...
        function processPolling(elt, handler, spec) {
            ...
            nodeData.timeout = setTimeout(function () {
                if (bodyContains(elt) && nodeData.cancelled !== true) {
                    if (!maybeFilterEvent(spec, makeEvent('hx:poll:trigger', {
                            triggerSpec: spec,
                            target: elt
                        }))) {
                        handler(elt);
        ...
        function boostElement(elt, nodeData, triggerSpecs) {
            if ((elt.tagName === "A" && isLocalLink(elt) && (elt.target === "" || elt.target ===
                    "_self")) || elt.tagName === "FORM") {
        ...
        function addEventListener(elt, handler, nodeData, triggerSpec, explicitCancel) {
            ...
            forEach(eltsToListenOn, function (eltToListenOn) {
                var eventListener = function (evt) {
                    if (!bodyContains(elt)) {
                        eltToListenOn.removeEventListener(triggerSpec.trigger,
                            eventListener);
                        return;
                    }
            ...
            if (!scrollHandler) {
                ...
                setInterval(function () {
                    if (windowIsScrolling) {
                        ...
                        forEach(getDocument().querySelectorAll(
        ...
        function ensureWebSocket(elt, wssSource, retryCount) {
            ...
            socket.addEventListener('message', function (event) {
                ...
                for (var i = 0; i < children.length; i++) {
                    ...
                    oobSwap(getAttributeValue(child, "hx-swap-oob") || "true", child,
                        settleInfo);
                }
        ...
        function processVerbs(elt, nodeData, triggerSpecs) {
            ...
            forEach(VERBS, function (verb) {
                if (hasAttribute(elt, 'hx-' + verb)) {
                    ...
                    nodeData.verb = verb;
                    triggerSpecs.forEach(function (triggerSpec) {
                        addTriggerHandler(elt, triggerSpec, nodeData, function (elt,
                            evt) {
                            issueAjaxRequest(verb, path, elt, evt)
                        })
                    });
        ...
        function addTriggerHandler(elt, triggerSpec, nodeData, handler) {
            if (triggerSpec.sseEvent) {
                ...
                if (!maybeFilterEvent(triggerSpec, makeEvent("load", {
                        elt: elt
                    }))) {
        ...
        function findElementsToProcess(elt) {
            if (elt.querySelectorAll) {
                ...
                var results = elt.querySelectorAll(VERB_SELECTOR + boostedElts +
        ...
        function loadHistoryFromServer(path) {
            ...
            request.onload = function () {
                if (this.status >= 200 && this.status < 400) {
                    ...
                    // @ts-ignore
                    fragment = fragment.querySelector('[hx-history-elt],[data-hx-history-elt]') ||
                        fragment;
        ...
        function shouldInclude(elt) {
            ...
            if (elt.type === "button" || elt.type === "submit" || elt.tagName === "image" || elt.tagName ===
                "reset" || elt.tagName === "file") {
                return false;
            }
            ...
            forEach(includes, function (node) {
                ...
                if (!matches(node, 'form')) {
                    forEach(node.querySelectorAll(INPUT_SELECTOR), function (descendant) {
                        processInputValue(processed, values, errors, descendant,
                            validate);
                    })
                }
            });
        ...
        function getSwapSpecification(elt, swapInfoOverride) {
            var swapInfo = swapInfoOverride ? swapInfoOverride : getClosestAttributeValue(elt,
                "hx-swap");
            var swapSpec = {
                ...
                var doSwap = function () {
                    try {
                        ...
                        if (selectionInfo.elt &&
                            ...
                            var focusOptions = {
                                preventScroll: swapSpec.focusScroll !== undefined ? !swapSpec.focusScroll :
        ...
        function insertIndicatorStyles() {
            if (htmx.config.includeIndicatorStyles !== false) {
                getDocument().head.insertAdjacentHTML("beforeend",
                    ...
                    "{opacity:1}\
                      ." + htmx.config.requestClass + "." + htmx.config
                    .indicatorClass + "{opacity:1}\
                    </style>");
            }
        }
