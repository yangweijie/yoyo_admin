...
class MvBuild extends \think\admin\Command
{
    ...
    protected function execute(Input $input, Output $output)
    {
        ...
        debug('begin');
        $new = Musics::where('playlist', $id)->field(['id', 'lrc', 'name', 'artist', 'pic', 'path', Db::raw('RANK() OVER (ORDER BY id ASC)')=>'rank'])->select();
            ...
            foreach ($todos as $item){
                try {
                    if(is_file($item['path'])){
                        ...
                        $this->generateCover(['cover'=>$cover, 'mp3'=>$item['path'], 'name'=>$target_name]);
                        if(!is_file($cover)){
